...
//!
//! This module implements a bottom-up economic simulation with two agent types:
//!
...
//! - `production/`  Recipe and facility definitions (WIP)
...
// - SettlementId, PopId, MerchantId (use pops::types::*)
// - Settlement, Route, NaturalResource (use pops::geography::*)
// - Stockpile (use pops::agents::Stockpile)
// - FacilityId, Facility (use pops::labor::FacilityId, pops::labor::Facility)

// Core types (only non-conflicting ones)
pub use types::{GoodId, GoodProfile, NeedContribution, Price, Quantity};

// Agents (only non-conflicting ones)
pub use agents::{ConsumptionResult, MerchantAgent, Pop};
